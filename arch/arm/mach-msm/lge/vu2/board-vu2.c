...
#if defined(CONFIG_BATTERY_MAX17043) || defined(CONFIG_LEDS_LP5521) /* jjm_rgb */	
/* LGE_CHANGE
...
#ifdef CONFIG_LGE_IRRC
/* LGE_CHANGE
 * Add to enable dual mode protocol of  gsbi4
 */
#define MSM_GSBI4_PHYS		0x16300000

static struct msm_serial_hslite_platform_data msm8960_uart_gsbi4_pdata = {
    .config_gpio    = 1,
    .uart_tx_gpio   = 18,
    .uart_rx_gpio   = 19,
    .line			= 1,
...
/* 20120704, sukkkong.kim@lge.com, Add IRRC UART [START] */
#ifdef CONFIG_LGE_IRRC
	{
		...
		void *gsbi4_mem = ioremap_nocache(MSM_GSBI4_PHYS, 4);
		writel_relaxed(GSBI_DUAL_MODE_CODE, gsbi4_mem);
		mb();
		iounmap(gsbi4_mem);
		msm8960_device_uart_gsbi4.dev.platform_data =
	           &msm8960_uart_gsbi4_pdata;
	}
#endif
/* 20120704, sukkkong.kim@lge.com, Add IRRC UART [END] */
}
...
static struct msm_i2c_platform_data msm8960_i2c_qup_gsbi4_pdata = {
	.clk_freq = 100000,  /* LGE_CHANGE, Change i2c clock due to camera preview crash issue, 2012-07-31 jeongda.lee@lge.com */
	.src_clk_rate = 24000000,
	.use_gsbi_shared_mode = 1,	//sukkkong.kim@lge.com IRRC gsbi4_DualMode
	.msm_i2c_config_gpio = gsbi_qup_i2c_gpio_config,
};
#else
static struct msm_i2c_platform_data msm8960_i2c_qup_gsbi4_pdata = {
// LGE_CHNAGE_S sungsik.kim 2012/08/19 {
// i2c clk 400KHz > 100KHz
	.clk_freq = 100000,
// LGE_CHNAGE_E sungsik.kim 2012/08/19 }
	.src_clk_rate = 24000000,
};
#endif
/* 20120704, sukkkong.kim@lge.com, Add IRRC UART [END] */
...
#if defined(CONFIG_LGE_NFC) || defined(CONFIG_MSM_CAMERA_FLASH_LM3559) || defined (CONFIG_RADIO_SI470X)
...
#elif defined(CONFIG_RADIO_SI470X)	//20120925 hanna.oh@lge.com Changing FM Radio i2C port  gsbi8 -> gsbi1
	&msm8960_device_qup_i2c_gsbi1,
#else
...
#if defined(CONFIG_FTT_CHARGER) //hyunjun.park
#include <mach/ftt_gpio.h>
#include <mach/ftt_charger.h>

char g_batt_charge_change = 1;

bool batt_charge_enable(bool enable)
{
//	int err;

	//err = gpio_request(FTT_CHARGE_EN, "FTT_CHARGE_IC");
	//if (err)
	//	printk("#### failed to request FTT_CHARGE_IC ####\n");
	//gpio_direction_output(FTT_CHARGE_EN, 1);
//	s3c_gpio_setpull(FTT_CHARGE_EN, S3C_GPIO_PULL_NONE); //hyunjun.park

	if (enable) {
		printk("%s = enable\n", __FUNCTION__);
		//gpio_set_value(FTT_CHARGE_EN, 0);
		g_batt_charge_change = 1;
	}
	else {
		printk("%s = disable\n", __FUNCTION__);
		//gpio_set_value(FTT_CHARGE_EN, 1);
		g_batt_charge_change = 0;
	}
	//gpio_free(FTT_CHARGE_EN);
...
static struct ftt_charger_pdata ftt_charger_data = {
	...
	.detect 	= FTT_DETECT,
	.ftt 	= FTT_FREQUANCY,
	.batt_charge_enable = batt_charge_enable,
};

static struct ftt_charger_pdata ftt_charger_data_RevA = {
	.en1 	= FTT_CHIP_ENABLE,
	.detect 	= FTT_DETECT,
	.ftt 	= FTT_FREQUANCY_RevA,
	.batt_charge_enable = batt_charge_enable,
};
...
#ifdef CONFIG_FTT_CHARGER
if (lge_get_board_revno() == HW_REV_A)
	ftt_charger_device.dev.platform_data = &ftt_charger_data_RevA;
#endif
...
MACHINE_START(MSM8960_VU2, "LGE MSM8960 VU2")
