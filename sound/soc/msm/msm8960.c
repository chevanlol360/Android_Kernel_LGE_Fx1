...
static atomic_t auxpcm_rsc_ref; /* 20121003 donggyun.kim - QCT workaround for BT Tx AUX */
...
static int msm8960_auxpcm_startup(struct snd_pcm_substream *substream)
{
	int ret = 0;
/*
*	20121003 donggyun.kim
*	QCT workaround for BT Tx AUX (case# 00976853)
*	the code below is 1732 original version
*
...
/* LGE_UPDATE 20120219 add feature for non-hdmi device */
	...
	},
	{
		...
		.be_hw_params_fixup = msm8960_auxpcm_be_params_fixup,
		.ops = &msm8960_auxpcm_be_ops, /* 20120927 donggyun.kim - QCT workaround for BT Tx AUX */
	},
...
*/
	...
	if (msm8960_configure_headset_mic_gpios()) {
		...
		msm8960_headset_gpios_configured = 1;
#endif
	atomic_set(&auxpcm_rsc_ref, 0); /* 20121003 donggyun.kim - QCT workaround for BT Tx AUX */
